...
def topsis(input_file, weights, impacts, output_file):
    ...
    # Reshape weights to match the number of columns in the decision matrix
    # Reshape weights to match the number of columns in the decision matrix
    ...
    # rank = len(performance_score) - np.argsort(performance_score) 
     ...
     # Determine the rank
    indices_descending = np.argsort(-performance_score)

    # Use the sorted indices to get the ranks in ascending order
    rank = np.argsort(indices_descending) + 1    
